...
function History() {
  ...
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);
   const  [activeNodeId, setIsActiveNodeId] = useState(null)
      ...
      <div className='grid grid-cols-1 lg:grid-cols-4 gap-6 '>
       ...
       {isSidebarOpen  &&
          <motion.div
           ...
           className='fixed lg:static top-0 left-0 z-50 lg:z-auto w-72 lg:w-auto h-full lg:h-[75vh]
           lg:col-span-1 bg-black/90 lg:bg-black/80 backdrop-blur-xl border border-white/10
           shadow-[0_20px_45px_rgba(0,0,0,0.6)] p-5 overflow-y-auto lg:rounded-3xl'>
            ...
            <div className='mb-4 space-y-1 '>
              <button onClick={()=>navigate("/notes")}
               className='w-full px-3 py-2 rounded-lg text-sm text-start text-gray-200 bg-white/10
              hover:bg-white/20'>
                ➕ New Notes
              </button>
        ...
        <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6 }}
        className='lg:col-span-3 rounded-2xl b-white shadow-[0_15px_40px_rgba(0,0,0,0.15)]
        p-6 min-[75vh]'>

          {loading && <p className='text-center text-gray-500'>Loading notes..</p>}
          {!loading && !selectedNote && (
            <div className='h-full flex items-center justify-center text-gray-400'>
                 Selected a topic from the Sidebar
            </div>
          )}
